...
export default function App() {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showToast(authMode === 'login' ? 'Logged in' : 'Registered', 'Welcome!');
      } else {
  ...
  return (
    <div className="min-h-screen relative selection:bg-lime selection:text-navy">
      ...
      <div className="pt-20">
        <Routes>
          <Route path="/" element={<Home skills={skills} isLoading={isLoading} user={user} token={token} showToast={showToast} setSkills={setSkills} />} />
          <Route path="/course/:id" element={<CourseDetail user={user} token={token} />} />
          <Route path="/how-it-works" element={<HowItWorks />} />
          <Route path="/categories" element={<Categories skills={skills} />} />
        </Routes>
      </div>
